...
class Problem(SearchProblem):
			...
			while i < len(state):
				...
				if (rx,ry) == (state[i][0],state[i][1]) and (rx,ry) != salida:
					...
					if ry != n:
						acciones.append((i, (rx, ry + 1, state[i][2])))
				i += 1
			#si hay un aparato en el mismo lugar, lo enfrio
			if extinguir:
	...
	def heuristic(self, state):
		...
		#rx,ry = state[0]
		#for ap in state[1:]:
		#	distanciaAMeta = n*2 - ap[0] - ap[1]
		#	distancia += distanciaAMeta
		menor = 0
		rx,ry = state[0]
		for ap in range(1,len(state)):
			if (state[ap][0],state[ap][1]) != salida:
				#distancia a la meta + distancia al robot
